...
LuminarView::LuminarView(int width, int height) : screen_width_(width), screen_height_(height) {
  ...
  for (int i = 0; i < screen_height_; ++i) {
    ...
    //std::fill(depth_buf_.begin(), depth_buf_.end(), 60); // 60 - far clipping
  }
}
...
void LuminarView::Draw(const Camera& cam, Mesh& mesh) {
  ...
  for (const auto& face : mesh.faces_) {
    ...
    float inv_z1 = 1 / mesh.vertices_[ind_1 - 1].z;
    float inv_z2 = 1 / mesh.vertices_[ind_2 - 1].z;
    float inv_z3 = 1 / mesh.vertices_[ind_3 - 1].z;
    float z1 = mesh.vertices_[ind_1 - 1].z;
    float z2 = mesh.vertices_[ind_2 - 1].z;
    float z3 = mesh.vertices_[ind_3 - 1].z;

    Vec3<float> v0_raster{static_cast<float>(raster_x1), static_cast<float>(raster_y1), inv_z1};
    Vec3<float> v1_raster{static_cast<float>(raster_x2), static_cast<float>(raster_y2), inv_z2};
    Vec3<float> v2_raster{static_cast<float>(raster_x3), static_cast<float>(raster_y3), inv_z3};  
      ...
      for (int x = 0; x < width; ++x) {
        ...
        if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
          ...
          if (depth < depth_buf_[y][x]) {
            ...
            float r = (w0 * c0.x + w1 * c1.x + w2 * c2.x) * depth;
            float g = (w0 * c0.y + w1 * c1.y + w2 * c2.y) * depth;
            float b = (w0 * c0.z + w1 * c1.z + w2 * c2.z) * depth;

            image_buf_[y][x] = Vec3<uint>{static_cast<uint>(r * 255), static_cast<uint>(g * 255), static_cast<uint>(b * 255)};
          }
        }
      }
    }
...
float LuminarView::EdgeFunction(const Vec3<float>& v0, const Vec3<float>& v1, 
